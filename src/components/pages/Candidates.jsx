...
const Candidates = () => {
    ...
    function color(str){
        if(str === "Completed")
            ...
            return 'text-[#991b1b] bg-[#fee2e2] border-[#fcafa5]'
    }
            ...
            <tbody>
                {   
                    results.slice(start, (start + pageLength)).map(result => {
                        return (    
                            <tr key={result.id} className='p-4 border-b border-b-gray-200'>
                                <td className='items  names'><b>{result.candidate_name}</b></td>
                                <td className='items  text-sm '>{result.job_title}</td>
                                <td className='items text-xs'>
                                    <b className={pickColor(result.status)}>{result.status}</b>
                                </td>
                                <td className='items text-sm'>{result.updatedAt.split("T")[0]}</td>
                                <td className='items'>
                                    <div className='flex cursor-pointer select-none text-[#3985b6] active:scale-99 acitve:text-[#3985b6]  hover:text-[#265c80] hover:scale-102 transition-all duration-100'>
                                        <img className="w-4 mr-2" src={eye} alt="view" />
                                        <span className="">
                                            View
                                        </span>
                                    </div>
