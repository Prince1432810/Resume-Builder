...
const PrimarySkills = ({data}) => {
  ...
  const { primarySkills, primarySkillName, primaryLevel, primarySkillsData, setPrimarySkills, setPrimarySkillName, setPrimaryLevel, setPrimarySkillsData } = data;
  ...
  const handleSaveEdit = () => {
    setPrimarySkillsData(prev => prev.map(item =>
  ...
  return (
    <div className=''>
              ...
              <div className='grid md:grid-cols-2 gap-x-5 gap-y-6 mb-6'>
                <div className='inputContainer'>
                  <div className='flex items-center mb-0.5 gap-1'>
                    ...
                    <label className='labelField' >Primary Skill Name</label>
                  </div>
                  <input
                    type="text"
                    value={editForm.primarySkillName || ''}
                    onChange={(e) => handleEditChange('primarySkillName', e.target.value)}
                ...
                <div className='inputContainer'>
                  ...
                  <select
                    ...
                    onChange={(e) => handleEditChange('primaryLevel', e.target.value)}
                    className='inputField'
                  >
      ...
      <div className={primarySkills ? 'p-5 border mt-5 border-gray-200 rounded-lg' : "hidden"}
        style={{ animation: 'slideDown 0.3s ease' }}
      >
        <div className='grid md:grid-cols-2 gap-x-5 gap-y-6 mb-6'>
          <div className='inputContainer'>
        ...
        <button
          onClick={() => { addSkill(); setPrimarySkills(false); }}
      ...
      }} className={primarySkills ? "hidden" : 'btn mt-5'}>Add Skill</button>
    </div>
  )
}
