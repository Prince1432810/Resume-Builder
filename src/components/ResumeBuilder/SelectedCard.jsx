...
const SelectedCard = ({ data, noShow, onEdit }) => {
    ...
    return isSkill ? (
        ...
        <div
            className={
                show
                    ? "relative w-full h-fit mt-4 bg-white border border-gray-300 rounded-xl overflow-hidden flex"
                    : "hidden"
            }
        >
            <button
            ...
            <div
                className={`w-1 shrink-0 rounded-l-xl ${primarySkillName ? "bg-[#5DCAA5]" : "bg-[#378ADD]"}`}
            ></div>
            ...
            ${primarySkillName ? "bg-[#e1f5ee] text-[#085041]" : "hidden"}`}
                    >
                        {level}
                    </span>
                    <button
        ...
        >
            ...
            <div className="flex flex-col gap-1 flex-1 p-4">
                <div className="flex justify-between items-start">
                    <div className="flex flex-col gap-0.5">
                        <span
                            className={
                                URL
                                    ? "text-md font-semibold text-[#3985B6]"
                                    : organization
                    ...
                    <button
                        onClick={() => noShow()}
                        className="z-10 px-1.5 py-0.5 ml-2 rounded-full bg-red-100 border border-red-500 text-red-500 text-xs flex-shrink-0"
                    >
                        x
                    </button>
                </div>
                ...
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {location && (
                        <div className="text-xs text-[#3986b5]">{location}</div>
                    )}
                </div>
            </div>
        </div>
