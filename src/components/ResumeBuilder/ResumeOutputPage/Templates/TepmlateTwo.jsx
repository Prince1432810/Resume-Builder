...
const TemplateTwo = (props) => {
  ...
  const hasSummary = summary && summary.replace(/<[^>]*>/g, "").trim().length > 0;
      ...
      >
        ...
        <div className="w-[260px] min-w-[260px] bg-[#0177B5] flex flex-col items-center px-5 py-8 self-stretch">
          ...
          <div className="w-[110px] h-[110px] rounded-full overflow-hidden border-[3px] border-white/60 shrink-0">
            <img src="/oggyFace.jpg" alt="" className="w-full h-full object-cover" />
          </div>
          {/* </div> */}
          ...
          <SbLine />
          {email && <SbRow icon={<MailIcon style={{ fontSize: "13px" }} />}>{email}</SbRow>}
          {phone && <SbRow icon={<CallIcon style={{ fontSize: "13px" }} />}>{phone}</SbRow>}
          {(city || state || country) && (
            <SbRow icon={<LocationOnIcon style={{ fontSize: "13px" }} />}>
              {[city, state, country].filter(Boolean).join(" , ")}
            </SbRow>
          )}
          {linkedin && <SbRow icon={<LinkedInIcon style={{ fontSize: "13px" }} />}>{linkedin}</SbRow>}
          ...
          {eduData.length > 0 && (
            <>
              ...
              <SbLine />
              {eduData.map((edu, idx) => (
                <div key={edu.id} className="w-full flex flex-row items-start gap-2 mb-[10px]">
                  ...
                  <div className="flex flex-col items-center shrink-0 h-full">
                    <div className="w-[30px] h-[30px] rounded-full bg-white/[0.18] flex items-center justify-center shrink-0 mt-[2px]">
                      <SchoolIcon style={{ fontSize: "15px", color: "#fff" }} />
                    </div>
                ...
                <div key={skill.id} className="w-full mb-[9px]">
                  ...
                  <div className="w-full h-[6px] bg-white/[0.22] rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: barWidths[idx % barWidths.length] }}
                    />
                  </div>
                </div>
              ))}
            ...
            <>
              ...
              {certificateData.map((cert, idx) => (
                <div key={cert.id} className="w-full flex flex-row gap-2">
                  ...
                  {/* Text */}
                  <div className={`flex-1 ${idx < certificateData.length - 1 ? "pb-[10px]" : ""}`}>
                    <div className="flex justify-between items-start">
                      <div>
...
const SbHeading = ({ children, top }) => (
  <p className={`text-white text-[11px] font-bold tracking-[1.6px] uppercase w-full mb-[5px] ${top ? "mt-5" : "mt-0"}`}>
    {children}
  </p>
);

const SbLine = () => (
  <div className="w-full h-px bg-white/35 mb-[9px]" />
);
